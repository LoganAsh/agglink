...
function roleBadgeClasses(role: string, facilityType?: string) {
  if (role === 'admin')      return 'bg-purple-500/20 text-purple-700 border border-purple-500/30';
  if (role === 'contractor') return 'bg-red-500/20 text-red-700 border border-red-500/30';
  if (role === 'trucking')   return 'bg-cyan-500/20 text-cyan-700 border border-cyan-500/30';
  if (role === 'supplier') {
    if (facilityType === 'dump') return 'bg-blue-500/20 text-blue-700 border border-blue-500/30';
    if (facilityType === 'both') return 'bg-emerald-500/20 text-emerald-700 border border-emerald-500/30';
...
export default function AdminView({
      ...
      <aside className="w-60 bg-white border-r border-zinc-200 hidden md:flex flex-col flex-shrink-0">
        ...
        <div className="p-4 border-t border-zinc-200">
          <div className="flex items-center mb-3">
            <div className="w-8 h-8 rounded-full bg-purple-500/20 border border-purple-500/40 flex items-center justify-center text-purple-700 font-bold text-xs">{adminName.substring(0, 2).toUpperCase()}</div>
            <div className="ml-2.5"><p className="text-xs font-semibold text-zinc-900">{adminName}</p><p className="text-[10px] text-purple-700 font-medium">Administrator</p></div>
          </div>
          <LogoutButton />
        </div>
      </aside>
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-zinc-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10 flex-shrink-0">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">AggLink platform administration</p>
            </div>
          </div>
          <a href="/dashboard" className="hidden md:block text-xs text-zinc-600 hover:text-orange-600 transition-colors border border-zinc-200 hover:border-orange-500/40 px-3 py-1.5 rounded-lg">&larr; Contractor View</a>
        </header>
        ...
        <div className="p-4 md:p-6 pb-24 md:pb-6 space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                  <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80"><h2 className="text-sm font-semibold text-zinc-900">Top Requested Materials</h2></div>
                  <div className="p-4 space-y-3">
                    {stats.topMaterials.length === 0 && <p className="text-zinc-500 text-sm text-center py-4">No data yet.</p>}
                    {stats.topMaterials.map(([name, data]) => {
                      const maxCount = stats.topMaterials[0]?.[1].count || 1;
                      const pct = (data.count / maxCount) * 100;
                      return (<div key={name}><div className="flex justify-between items-center mb-1"><span className="text-xs text-zinc-700 font-medium truncate max-w-[60%]">{name}</span><div className="flex items-center space-x-3"><span className="text-xs text-zinc-500">{data.count}x</span><span className="text-xs font-semibold text-orange-600">{fmtCurrency(data.avgPrice)}/unit</span></div></div><div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden"><div className="h-full bg-orange-500 rounded-full" style={{ width: `${pct}%` }}></div></div></div>);
                    })}
                  </div>
                </div>
                <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                  <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80"><h2 className="text-sm font-semibold text-zinc-900">Most Used Facilities</h2></div>
                  <div className="p-4 space-y-3">
                    {stats.topFacilities.filter(f => f.usageCount > 0).length === 0 && <p className="text-zinc-500 text-sm text-center py-4">No estimate data yet.</p>}
                    {stats.topFacilities.filter(f => f.usageCount > 0).map((fac) => {
                      ...
                      const typeColor = fac.type === 'pit' ? 'text-orange-600' : fac.type === 'dump' ? 'text-blue-700' : 'text-emerald-700';
                      const barColor  = fac.type === 'pit' ? 'bg-orange-500' : fac.type === 'dump' ? 'bg-blue-500' : 'bg-emerald-500';
                      return (<div key={fac.id}><div className="flex justify-between items-center mb-1"><span className="text-xs text-zinc-700 font-medium truncate max-w-[65%]">{fac.name}</span><div className="flex items-center space-x-2"><span className={`text-[10px] uppercase font-bold ${typeColor}`}>{fac.type}</span><span className="text-xs text-zinc-500">{fac.usageCount}x</span></div></div><div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${barColor}`} style={{ width: `${pct}%` }}></div></div></div>);
                    })}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'requests' && (
            <div className="space-y-4">
              {actionError && <div className="bg-red-500/10 border border-red-500/30 text-red-700 text-sm px-4 py-3 rounded-lg">{actionError}</div>}
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                      ...
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center space-x-2">
                            <span className="font-semibold text-zinc-900 text-sm">{req.full_name}</span>
                            <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider ${req.requested_role === 'contractor' ? 'bg-red-500/20 text-red-700' : req.requested_role === 'trucking' ? 'bg-cyan-500/20 text-cyan-700' : 'bg-orange-500/20 text-orange-600'}`}>{req.requested_role}</span>
                          </div>
                          <p className="text-xs text-zinc-600">{req.email}</p>
                          <p className="text-xs text-zinc-600">{req.company_name}</p>
                          {req.notes && <p className="text-xs text-zinc-500 italic mt-1">&ldquo;{req.notes}&rdquo;</p>}
                          <p className="text-xs text-zinc-400 mt-1">Submitted {fmtDate(req.created_at)}</p>
                        </div>
              ...
              {signupRequests.filter(r => r.status !== 'pending').length > 0 && (
                <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                  <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80"><h2 className="text-sm font-semibold text-zinc-900">Reviewed</h2></div>
                  <div className="divide-y divide-zinc-200/70">
                    {signupRequests.filter(r => r.status !== 'pending').map(req => (
                      <div key={req.id} className="px-5 py-3 flex items-center justify-between">
                        <div><div className="flex items-center space-x-2"><span className="text-sm text-zinc-700 font-medium">{req.full_name}</span><span className="text-xs text-zinc-500">{req.company_name}</span></div><p className="text-xs text-zinc-500">{req.email}</p></div>
                        <div className="flex items-center space-x-2">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider ${req.status === 'approved' ? 'bg-emerald-500/20 text-emerald-700' : 'bg-red-500/20 text-red-700'}`}>{req.status}</span>
                          {req.reviewed_at && <span className="text-xs text-zinc-400">{fmtDate(req.reviewed_at)}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'users' && (
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-zinc-900">All Users ({profiles.length})</h2>
                    ...
                    {profiles.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-zinc-100 transition-colors">
                          ...
                          <td className="px-5 py-3 text-zinc-600">{p.created_at ? fmtDate(p.created_at) : '-'}</td>
                          <td className="px-5 py-3">
                            {isEditing ? (
                              <div className="flex items-center space-x-2">
                                <select defaultValue={p.role} disabled={isSaving} onChange={(e) => handleRoleChange(p.id, e.target.value as Role)} className="bg-zinc-100 border border-zinc-300 rounded-md px-2 py-1 text-xs text-zinc-900 focus:outline-none focus:border-orange-500 disabled:opacity-50">
                                ...
                                <button onClick={() => setEditingRoleId(null)} disabled={isSaving} className="text-zinc-500 hover:text-zinc-700 text-xs transition-colors">Cancel</button>
                              </div>
                            ) : (
                              <button onClick={() => setEditingRoleId(p.id)} className="text-xs text-zinc-500 hover:text-orange-600 transition-colors border border-zinc-200 hover:border-orange-500/40 px-2 py-1 rounded-md">{isSaving ? 'Saving...' : 'Change'}</button>
                            )}
                          </td>
                        </tr>
                      );
          ...
          {activeTab === 'projects' && (
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80"><h2 className="text-sm font-semibold text-zinc-900">All Projects ({projects.length})</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    ...
                    {projects.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-zinc-100 transition-colors">
                          ...
                          <td className="px-5 py-3 text-zinc-600">{projEstimates.length}</td>
                          <td className="px-5 py-3 text-right font-semibold text-emerald-700">{projValue > 0 ? fmtCurrency(projValue) : '-'}</td>
                          <td className="px-5 py-3 text-zinc-600">{fmtDate(p.created_at)}</td>
                        </tr>
                      );
                    })}
          ...
          {activeTab === 'facilities' && (
            ...
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-zinc-900">All Facilities ({facilities.length})</h2>
  ...
  <select
    ...
    className="bg-white border border-zinc-200 rounded-md px-2 py-1 text-xs text-zinc-900 focus:outline-none focus:border-orange-500 disabled:opacity-50"
  >
  ...
  {assigningFacilityId === f.id && <span className="ml-2 text-xs text-zinc-600">Saving...</span>}
            ...
            <div className="bg-white border border-zinc-200 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-zinc-900 mb-4">Add New Facility</h3>
              <form onSubmit={createFacility} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs text-zinc-600 mb-1">Type</label>
                ...
                <div>
                  <label className="block text-xs text-zinc-600 mb-1">Latitude (optional)</label>
                  <input
                ...
                <div>
                  <label className="block text-xs text-zinc-600 mb-1">Longitude (optional)</label>
                  <input
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs text-zinc-600 mb-1">Owner</label>
                  <select
          ...
          {activeTab === 'materials' && (
            <div className="space-y-6">
              ...
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-zinc-500 bg-zinc-50 uppercase tracking-wider"><tr><th className="px-5 py-3">Material</th><th className="px-5 py-3">Facility</th><th className="px-5 py-3">Type</th><th className="px-5 py-3 text-right">Price / Ton</th><th className="px-5 py-3">Est. Requests</th></tr></thead>
                    <tbody className="divide-y divide-zinc-200/70">
                      {materials.map(m => {
          ...
          {activeTab === 'categories' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                  ...
                  <div className="divide-y divide-zinc-200/70">
                    ...
                    {categories.map(c => (
                      <div key={c.id} onClick={() => setSelectedCatId(selectedCatId === c.id ? null : c.id)}
                        ...
                        <div className="flex items-center space-x-3">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider ${c.type === 'import' ? 'bg-orange-500/20 text-orange-600' : 'bg-blue-500/20 text-blue-700'}`}>{c.type}</span>
                          <span className="text-sm text-zinc-900 font-medium">{c.name}</span>
                          <span className="text-xs text-zinc-500">{categoryMap.filter(m => m.category_id === c.id).length} materials</span>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); deleteCategory(c.id); }} className="text-zinc-400 hover:text-red-500 transition-colors text-xs"><i className="fa-solid fa-trash"></i></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Create category */}
                <div className="bg-white border border-zinc-200 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-zinc-900 mb-3">Create Category</h3>
              ...
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-zinc-200 bg-zinc-50/80">
                  <h2 className="text-sm font-semibold text-zinc-900">{selectedCat ? `Assign Materials to "${selectedCat.name}"` : 'Select a category to assign materials'}</h2>
                </div>
                {!selectedCat ? (
                  <p className="px-5 py-8 text-center text-zinc-500 text-sm">Click a category on the left to manage its materials.</p>
                ) : availableForCat.length === 0 ? (
                  <p className="px-5 py-8 text-center text-zinc-500 text-sm">No {selectedCat.type} materials found in the database.</p>
                ) : (
                    ...
                    {availableForCat.map(mat => {
                      ...
                      return (
                        <button key={mat} onClick={() => toggleMaterialAssignment(selectedCatId!, mat)} disabled={assigningMat}
                          className={`flex items-center space-x-3 px-3 py-2 rounded-lg border text-left transition-all disabled:opacity-50 ${isAssigned ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-800' : 'bg-zinc-50/80 border-zinc-200 text-zinc-600 hover:border-zinc-400 hover:text-zinc-900'}`}>
                          <span className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${isAssigned ? 'bg-emerald-500 border-emerald-500' : 'border-zinc-300'}`}>
                            {isAssigned && <i className="fa-solid fa-check text-zinc-900" style={{ fontSize: '9px' }}></i>}
                          </span>
          ...
          {activeTab === 'trucks' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="divide-y divide-zinc-200/70">
                  ...
                  {truckTypes.map(t => (
                    <div key={t.id} className="px-5 py-3 flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <i className="fa-solid fa-truck text-zinc-500 text-sm"></i>
                        <span className={`text-sm font-medium ${t.active ? 'text-zinc-900' : 'text-zinc-500 line-through'}`}>{t.name}</span>
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider ${t.active ? 'bg-emerald-500/20 text-emerald-700' : 'bg-zinc-200 text-zinc-500'}`}>{t.active ? 'Active' : 'Inactive'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <button onClick={() => toggleTruckActive(t)} className="text-xs text-zinc-600 hover:text-orange-600 border border-zinc-200 hover:border-orange-500/40 px-2 py-1 rounded-md transition-all">{t.active ? 'Disable' : 'Enable'}</button>
                        <button onClick={() => deleteTruckType(t.id)} className="text-zinc-400 hover:text-red-500 transition-colors text-xs"><i className="fa-solid fa-trash"></i></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white border border-zinc-200 rounded-xl p-5">
                <h3 className="text-sm font-semibold text-zinc-900 mb-4">Add Truck Type</h3>
      ...
      {showMoreMenu && (
        <div className="md:hidden fixed inset-0 z-40" onClick={() => setShowMoreMenu(false)}>
          <div className="absolute bottom-16 left-0 right-0 bg-white border-t border-zinc-200 rounded-t-2xl shadow-2xl p-4"
            onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 bg-zinc-100 rounded-full mx-auto mb-4"></div>
            <p className="text-xs text-zinc-500 font-semibold uppercase tracking-wider mb-3 px-2">More</p>
            <div className="space-y-1">
              {[
                ...
                <button key={t.id} onClick={() => { setActiveTab(t.id as Tab); setShowMoreMenu(false); }}
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${activeTab === t.id ? 'bg-orange-500/10 text-orange-600' : 'text-zinc-700 hover:bg-white'}`}>
                  <i className={`fa-solid ${t.icon} w-5 text-center`}></i>
                  <span className="text-sm font-medium">{t.label}</span>
                  {activeTab === t.id && <i className="fa-solid fa-check ml-auto text-orange-600 text-xs"></i>}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-zinc-200 pb-safe">
        <div className="flex items-center justify-around px-2 py-2">

          {/* Overview */}
          <button onClick={() => { setActiveTab('overview'); setShowMoreMenu(false); }}
            className={`flex flex-col items-center space-y-1 px-3 py-1.5 rounded-lg transition-all ${activeTab === 'overview' ? 'text-orange-600' : 'text-zinc-500'}`}>
          ...
          <button onClick={() => { setActiveTab('requests'); setShowMoreMenu(false); }}
            className={`flex flex-col items-center space-y-1 px-3 py-1.5 rounded-lg transition-all relative ${activeTab === 'requests' ? 'text-orange-600' : 'text-zinc-500'}`}>
          ...
          <button onClick={() => { setActiveTab('users'); setShowMoreMenu(false); }}
            className={`flex flex-col items-center space-y-1 px-3 py-1.5 rounded-lg transition-all ${activeTab === 'users' ? 'text-orange-600' : 'text-zinc-500'}`}>
          ...
          <button onClick={() => { setActiveTab('facilities'); setShowMoreMenu(false); }}
            className={`flex flex-col items-center space-y-1 px-3 py-1.5 rounded-lg transition-all ${activeTab === 'facilities' ? 'text-orange-600' : 'text-zinc-500'}`}>
          ...
          <button onClick={() => setShowMoreMenu(!showMoreMenu)}
            className={`flex flex-col items-center space-y-1 px-3 py-1.5 rounded-lg transition-all ${showMoreMenu || ['projects','materials','categories','trucks','quotes'].includes(activeTab) ? 'text-orange-600' : 'text-zinc-500'}`}>
