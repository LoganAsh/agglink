...
export default function AdminView({
  ...
  const stats = useMemo(() => {
    ...
    projects.forEach(p => {
      projectsByContractor[p.contractor_id] = (projectsByContractor[p.contractor_id] || 0) + 1;
    });
        ...
        <div className="p-6 space-y-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              {/* Two-col: top materials + top facilities */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    {stats.topMaterials.map(([name, data], idx) => {
                    ...
                    {stats.topFacilities.filter(f => f.usageCount > 0).map((fac) => {
                      ...
                      return (
                        <div key={fac.id}>
                          ...
                          <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all ${fac.type === 'pit' ? 'bg-orange-500' : fac.type === 'dump' ? 'bg-blue-500' : 'bg-emerald-500'}`} style={{ width: `${pct}%` }}></div>
                          </div>
                        </div>
                      );
                    })}
